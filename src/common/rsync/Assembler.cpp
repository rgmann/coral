   ...
   if ( instruction.deserialize( stream ) )
   {
      if ( instruction.chunk_data_ptr_ )
      {
         ...
         if ( ( instruction.chunk_size_bytes_ > 0 ) &&
              ( instruction.chunk_size_bytes_ <= assembler_state_.descriptor().getMaximumChunkSize() ) )
         {
            assembler_state_.stream().write( (char*)instruction.chunk_data_ptr_, instruction.chunk_size_bytes_ );
...
void Assembler::processSegmentInstruction( SerialStream& stream )
{
   ...
   if ( instruction.deserialize( stream ) )
   {
      Segment* segment_ptr = assembler_state_.segmentAccessor().getSegment( instruction.segment_id_ );

      if ( segment_ptr != NULL )
      {
         assembler_state_.stream().write( (char*)segment_ptr->data(), segment_ptr->size() );
