...
//      l_bFound = pSegTable->find(**l_iCurrentSeg, l_fAuthFile, l_pSegPkt);
      ...
      if (l_pSegPkt)
      {
         ...
         // We have to shift the iterator by the size of the segment because
         // because a match means that the next few bytes (l_nSigSizeBytes to
         // be exact) can be found in the identified segment.
         for (int l_nShifts = 0;
