...
void run() {
    ...
    BSONObj p = BSON( "name" << "Joe" << "age" << 33 << "visits" << 1 );
    c.insert("tutorial.persons", p);
    p = BSON( "name" << "Jane" << "age" << 40 << "visits" << 5 );
    c.insert("tutorial.persons", p);
    p = BSON( "name" << "Abe" << "age" << 33 << "visits" << 2);
    c.insert("tutorial.persons", p);
    p = BSON( "name" << "Methuselah" << "age" << BSONNULL << "visits" << 1000);
    c.insert("tutorial.persons", p);
    p = BSON( "name" << "Samantha" << "age" << 21 << "city" << "Los Angeles" << "state" << "CA" << "visits" << 11 );
    c.insert("tutorial.persons", p);
